...
class TestExporter(tests.TestCase):
    ...
    def test_export_seedset(self, mock_producer_cls, mock_api_client_cls):
        ...
        mock_api_client.warcs.assert_called_once_with(seedset_id="005b131f5f854402afa2b08a4b7ba960", seed_ids = [], harvest_date_start=harvest_date_start, harvest_date_end=harvest_date_end)
    ...
    # Mock out Producer
    @patch("sfmutils.consumer.Producer", autospec=True)
    def test_export_seeds(self, mock_producer_cls, mock_api_client_cls):
        mock_warc_iter_cls = MagicMock()
        mock_table_cls = MagicMock()
        mock_table = MagicMock(spec=BaseTable)
        mock_table_cls.side_effect = [mock_table]
        ...
        mock_api_client_cls.assert_called_once_with("http://test")
        mock_api_client.warcs.assert_called_once_with(seedset_id=None, seed_ids = ["005b131f5f854402afa2b08a4b7ba960", "105b131f5f854402afa2b08a4b7ba960"], harvest_date_start=None, harvest_date_end=None)
    ...
    # Mock out Producer
    @patch("sfmutils.consumer.Producer", autospec=True)
    def test_export_seedset_missing_warc(self, mock_producer_cls, mock_api_client_cls):
        mock_api_client = MagicMock(spec=ApiClient)
        mock_api_client_cls.side_effect = [mock_api_client]
    ...
    def test_export_seedset_and_seeds(self, mock_producer_cls, mock_api_client_cls):
        mock_api_client = MagicMock(spec=ApiClient)
        mock_api_client_cls.side_effect = [mock_api_client]
        warcs = [{"warc_id":"9dc0b9c3a93a49eb8f713330b43f954c","path":"xtest_1-20151202165907873-00000-306-60892de9dfc6-8001.warc.gz","sha1":"000ffb3371eadb507d77d181ca3f0c5d3c74a2fc","bytes":460518,"date_created":"2016-02-22T14:49:07Z"}]
        ...
        mock_api_client_cls.assert_called_once_with("http://test")
        mock_api_client.warcs.assert_called_once_with(seedset_id="005b131f5f854402afa2b08a4b7ba960", seed_ids = [], harvest_date_end=None, harvest_date_start=None)
    ...
    def test_export_full_json(self):
        ...
        mock_warc_iter.iter.return_value = [(None, None, None, {"key1": "k1v1", "key2": "k2v1", "key3": "k3v1"}), (None, None, None, { "key1": "k1v2", "key2": "k2v2", "key3": "k3v2"})]

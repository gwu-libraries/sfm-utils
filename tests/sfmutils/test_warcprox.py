...
class WarcedTest(TestCase):
    ...
    def test_generate_commandline(self):
        w = warced("test", "/test")
        self.assertEqual("warcprox -c {} --certs-dir {} --dedup-db-file /dev/null --stats-db-file /dev/null "
                         "--max-threads=1 -d /test -n test -p {} -z".format(
            w.ca_bundle, w.ca_dir, w.port), w._generate_commandline())

        w = warced("test", "/test", compress=False, interrupt=True, rollover_time=60)
        self.assertEqual(
            "warcprox -c {} --certs-dir {} --dedup-db-file /dev/null --stats-db-file /dev/null --max-threads=1 "
            "-d /test -n test -p {} -i --rollover-time 60".format(w.ca_bundle, w.ca_dir, w.port),

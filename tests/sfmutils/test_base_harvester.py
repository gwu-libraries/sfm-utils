...
class TestBaseHarvester(TestCase):
    ...
    def test_harvest_from_file(self, mock_warced_class, mock_tempfile):
        ...
        #Create harvester and invoke _callback
        harvester = TestableHarvester(mock_state_store, test_warc_path)
        harvest_result = harvester.harvest_from_file(message_filepath)
        ...
        self.assertSetEqual({"http://www.gwu.edu", "http://library.gwu.edu"}, harvest_result.urls_as_set())
        self.assertEqual(1, len(harvest_result.infos))
        self.assertDictEqual({
            "code": "FAKE_CODE1",
            "message": "This is my message."
        }, harvest_result.infos[0].to_map())
        self.assertEqual(1, len(harvest_result.warnings))
        self.assertDictEqual({
            "code": "FAKE_CODE2",
            "message": "This is my warning."
        }, harvest_result.warnings[0].to_map())
        self.assertEqual(1, len(harvest_result.errors))
        self.assertDictEqual({
            "code": "FAKE_CODE3",
            "message": "This is my error."
        }, harvest_result.errors[0].to_map())
        self.assertIsNotNone(harvest_result.started)
        self.assertIsNotNone(harvest_result.ended)
        self.assertDictEqual({
            "photo": 12,
            "user": 1
        }, harvest_result.summary)
        self.assertDictEqual({
            "131866249@N02": "j.littman"
        }, harvest_result.token_updates)
        self.assertDictEqual({
            "library_of_congress": "671366249@N03"
        }, harvest_result.uids)
        self.assertListEqual([os.path.join(test_collection_path,
                              "2015/11/09/19/test_1-20151109195229879-00000-97528-GLSS-F0G5RP-8000.warc.gz")],
                             harvest_result.warcs)
